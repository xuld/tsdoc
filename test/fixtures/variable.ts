 ...
 */
export function setAttr(elem: HTMLElement, attrName: string, value: string | boolean | null) {
    if (value === null || value === false) {
        elem.removeAttribute(attrName);
    } else {
        elem.setAttribute(attrName, value === true ? attrName : value);
    }
}
...
export function on(elem: HTMLElement | Document, eventName: string, selector: string | typeof listener, listener?: ((e: Event, target?: HTMLElement | Document) => void) | typeof scope, scope?: any) {
    ...
    // 同一个监听器不重复执行。
    if (!listeners || indexOfListener(listeners, listener) < 0) {
        ...
        // 1. 事件委托，需要重新定位目标元素。
        // 2. 需要重写回调函数中的 this。
        // 3. 事件有特殊过滤器，仅在满足条件时触发。
        // 4. 监听器具有第二参数。
        if (selector || scope || bindFix.filter || listener.length > 1) {
            const orignalListener = listener;
            listener = function (this: HTMLElement, e: Event) {
                let target = this;
                ...
                orignalListener.call(scope || this, e, target);
            };
            (listener as any).__listener__ = orignalListener;
        }
 ...
 */
export function off(elem: HTMLElement | Document, eventName: string, selector: string, listener?: (e: Event, target: HTMLElement | Document) => void): void;
 ...
 */
export function off(elem: HTMLElement | Document, eventName: string, listener?: (e: Event, target: HTMLElement | Document) => void): void;

export function off(elem: HTMLElement | Document, eventName: string, selector?: string | typeof listener, listener?: (e: Event, target: HTMLElement | Document) => void) {
    if (typeof selector !== "string") {
        listener = selector;
        selector = "";
    }
    ...
    if (listeners) {
        if (listener) {
            ...
            for (listener of listeners) {
                off(elem, eventName, selector, listener);
            }
        } else {
            off(elem, eventName, selector, listeners);
        }
    }
}

function indexOfListener(listeners: typeof listener[] | typeof listener | undefined, listener: (e: Event, target: HTMLElement | Document) => void) {
    if (Array.isArray(listeners)) {
        for (let i = 0; i < listeners.length; i++) {
            if (listeners[i] === listener || (listeners[i] as any).__listener__ === listener) {
                return i;
            }
        }
        return -1;
    }
    return listeners === listener || (listeners as any).__listener__ === listener ? 0 : -1;
}
 ...
 * @param ease 渐变函数。可以使用 CSS3 预设的特效渐变函数。
 * @example animate(document.body, { height: 400 });
 */
export function animate(elem: HTMLElement, propNames: { [propName: string]: string | number }, duration = 150, callback?: (this: typeof elem) => void, ease = "ease-in") {
    if (!animateFix) {
    ...
    if (animateFix.support && duration !== 0) {
        const context = (elem.style as any)._animate || ((elem.style as any)._animate = { __proto__: null });
        const updateTransition = () => {
            let transition = "";
            for (const key in context) {
                if (transition) transition += ",";
                transition += `${key.replace(/[A-Z]|^ms|^webkit/g, word => "-" + word.toLowerCase())} ${duration}ms ${ease}`;
            }
        ...
        const transitionEnd = (e: Event) => {
            ...
            if (timer && (!e || e.target === e.currentTarget)) {
                ...
                for (const key in context) {
                    // 如果正在执行渐变的属性没有被新渐变覆盖，说明当前属性已执行渐变结束。
        ...
        let timer: number = setTimeout(transitionEnd, duration) as any;
    } else {
 ...
 * 判断指定的元素是否被隐藏。
 * @param elem 要判断的元素。
 * @return 如果当前元素被隐藏或正在被隐藏则返回 true，否则返回 false。
 ...
 * 切换元素显示的特效动画。
 */
export type ToggleAnimation = "opacity" | "height" | "width" | "top" | "bottom" | "left" | "right" | "scale" | "scaleX" | "scaleY" | "rotateX" | "rotateY" | typeof toggleAnimations[""];
 ...
 * @param ease 渐变函数。可以使用 CSS3 预设的特效渐变函数。
 */
export function show(elem: HTMLElement, animation?: ToggleAnimation, duration?: number, callback?: (this: typeof elem) => void, ease?: string) {
    if (animation || callback) {
        toggle(elem, animation, duration, callback, ease, true);
    } else {
        elem.style.display = (elem.style as any).__display__ || "";
        // 如果清空内联 display 后 display 仍然为 none, 说明通过 CSS 设置了 display 属性。
        // 将元素强制设为 inline 或 block。
 ...
 * @param ease 渐变函数。可以使用 CSS3 预设的特效渐变函数。
 */
export function hide(elem: HTMLElement, animation?: ToggleAnimation, duration?: number, callback?: () => void, ease?: string) {
    if (animation || callback) {
        toggle(elem, animation, duration, callback, ease, false);
    } else {
        const currentDisplay = getStyle(elem, "display");
        if (currentDisplay !== "none") {
            (elem.style as any).__display__ = currentDisplay;
 ...
 */
export function toggle(elem: HTMLElement, animation?: ToggleAnimation, duration?: number, callback?: (this: typeof elem) => void, ease?: string, value?: boolean): void;
 ...
 * @param value 如果为 true 则显示元素；如果为 false 则隐藏元素。
 */
export function toggle(elem: HTMLElement, value?: boolean): void;

export function toggle(elem: HTMLElement, animation?: ToggleAnimation | boolean, duration?: number, callback?: (this: typeof elem) => void, ease?: string, value?: boolean) {
    ...
    if (typeof animation === "string") {
        if (!toggleAnimations) {
            toggleAnimations = {
                ...
                right: { transform: "translateX(300%)" },
                rotateX: { transform: "rotateX(90deg)" },
    ...
    if (animation && duration !== 0) {
        if (value) {
            hidden && show(elem);
            const orignalAnimation = animation;
            animation = {};
            for (let key in orignalAnimation) {
                if (key === "overflow") {
                    animation[key] = orignalAnimation[key];
                } else {
                    key = vendor(key);
                    if ((elem.style as any).__toggle__ == undefined) {
                        // 正在执行动画，无需覆盖。
                        animation[key] = getStyle(elem, key);
                        setStyle(elem, key, orignalAnimation[key]);
                    } else {
                        const old = elem.style[key as any];
                        elem.style[key as any] = "";
                        animation[key] = getStyle(elem, key);
                        elem.style[key as any] = old;
                    }
                }
            }
        }
        (elem.style as any).__toggle__ = value;
        animate(elem, animation, duration, () => {
            for (const key in animation as typeof toggleAnimations[""]) {
                setStyle(elem, key, "");
            }
